...
;(function (global) {
  ...
  let //this will be used by the user.
    dycalendar = {},
    //window document
  ...
  function createMonthTable(data, option) {
    ...
    for (c = 0; c <= 6; c = c + 1) {
      ...
      td.innerHTML = dayName.ddd[c] //done
      tr.appendChild(td)
    }
    // сверху ячейки с названием месяцев
    ...
    while (c <= 6) {
      ...
      if (
        ...
        // td.setAttribute('class', 'dycalendar-today-date') close
      }
      if (
        option.date === count &&
        ...
        // td.setAttribute('class', 'dycalendar-target-date') close
      }
      tr.appendChild(td)
    ...
    table.appendChild(tr)
    //при наведении на td добавляется класс dycalendar-target-date
...
console.log(tdcolor)
function Color() {
  tdcolor.forEach((tdcolo) => {
    tdcolo.classList.add('dycalendar-today-date')
  })
}
Color()
